...
- `POST` **`/api/hit`** — 공개. `{ "page": "<슬러그>" }`. 슬러그 화이트리스트(11개): jara-holdem/roulette/whos-the-nut/icm-split/safe-link/cannon/jamakase/birthday/word-guesser/word-finder/smtm. 화이트리스트 외 → 400 `bad_page`. `page_views(page, ip_hash, day)` 1행 기록(`day`=KST 날짜). 메인 앱의 `AppWrapper`/`HtmlAppPage` 진입 시 호출.
