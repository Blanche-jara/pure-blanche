...
3. **쌍별 상계**: `net(A→B) = 미정산(A→B) − 미정산(B→A) − 선입금(A→B)`.
   음수면 화살표가 뒤집힌다(과다 송금 → 돌려받을 돈).

**갚는 방법은 둘, 장부는 하나**
- **건별**: 인원별 탭에서 건마다 `입금했습니다`.
- **합계**: 통합 탭 화살표의 `입금 기록` — 상계된 최종 금액이 미리 채워진다.

두 경로가 따로 놀면 같은 돈이 두 번 빠진다. 그래서 **송금을 기록하면 그 돈이 덮는 건을
오래된 순으로 함께 정산 처리**하고(`coverageOf`), 상계에서는 **건에 붙지 않고 남은
금액(`Transfer.credit = amount - applied`)만** 다시 뺀다. 건은 쪼개지 않으므로
모자란 송금은 통째로 선입금으로 남는다.

진행률(`settledRatio`)도 건 개수가 아니라 **남은 송금액 기준**이라, 어떤 방식으로 갚든
서로 빚이 상쇄되든 완료 판정(`isFullySettled`)과 항상 같은 얘기를 한다.
...
**비밀 프로젝트**: 생성 시 암호를 걸면 링크를 알아도 암호를 넣어야 열린다.
PBKDF2-SHA256 10만 회로 해시만 저장하고, 통과하면 받은 접근 토큰을 localStorage에 보관해
다음부터는 안 묻는다. 조회·편집 **양쪽 다** 열쇠가 필요하다.

**모바일 우선**: 600px 미만에서 여백·글자를 줄이고 인원 요약을 한 줄 행으로,
건별 행을 2줄 구조로 바꾼다. `test/settlement_mobile_layout_test.dart` 가
320~430px에서 오버플로가 없는지 검사한다.
